...
from trl import ModelConfig, ScriptArguments, TrlParser, create_reference_model, get_peft_config, CPOConfig, CPOTrainer
...
def train():
    parser = HfArgumentParser((DPOScriptArguments, CPOConfig, ModelConfig))
    ...
    train_result = cpo_trainer.train()
    metrics = train_result.metrics
    metrics["train_samples"] = len(train_dataset)
    cpo_trainer.log_metrics("train", metrics)
    cpo_trainer.save_metrics("train", metrics)
    cpo_trainer.save_state()
